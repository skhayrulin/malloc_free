...
p_q get_dev_queue()
{
    ...
    init_cl_devices(q);
    if (q.size() < 1) {
        throw "No OpenCL devices were found";
    }
    return q;
}
...
void init_devices(cl::Platform& p, p_q& q)
{
    cl_int err;
    cl::Context context;
...
void init_cl_devices(p_q& q)
{
    ...
    if (platform_list.size() < 1 || err != CL_SUCCESS) {
        throw "No OpenCL platforms were found";
    }
    // std::for_each(platform_list.begin(), platform_list.end(),
    // show_platform_info);
    auto it = std::max_element(platform_list.begin(), platform_list.end(),
