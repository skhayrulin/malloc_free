...
template <class T, size_t dim = 4> struct particle {
...
template <size_t dim = 4> struct particle_f : public particle<float, dim> {
protected:
  char _aligment[8];
};
...
std::vector<particle_f<>> particles(size);
...
void init_ocl_stuff() {
  create_ocl_buffer("particles", cl_particles, CL_MEM_READ_WRITE,
                    particles.size() * sizeof(particle_f<>));
...
unsigned int _run_work_with_struct() {
  // Stage HashParticles
  work_with_struct.setArg(0, cl_particles);
...
int main() {
  cout << "SIZEOF PARTICLE STRUCT FLOAT IS " << sizeof(particle_f<>) << endl;
